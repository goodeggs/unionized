...
module.exports = FactoryDefinition = (function() {
  ...
  FactoryDefinition.prototype.setAttrs = function() {
    ...
    for (path in _ref) {
      ...
      if (value !== void 0) {
        if (path.substr(-2) === '[]') {
          _results.push(this._arraySizes[path.slice(0, -2)] = value);
        } else {
  ...
  FactoryDefinition.prototype.embed = function(path, factory, callback) {
    ...
    return factory[this.mode]((_ref = this.get(path)) != null ? _ref : {}, (function(_this) {
  ...
  FactoryDefinition.prototype.embedArray = function(path, defaultCount, factory, callback) {
    var count, embedInstance, _ref;
    if (dotpath.containsSubpath(this.attrs, path)) {
      return _.defer(callback);
    }
    count = (_ref = this._arraySizes[path]) != null ? _ref : defaultCount;
    embedInstance = (function(_this) {
      return function(index, done) {
        return _this.embed("" + path + "[" + index + "]", factory, done);
      };
    })(this);
    return _.asyncRepeat(count, embedInstance, callback);
  };
